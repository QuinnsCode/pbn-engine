...
#[inline]
fn match_all_around(img: &[u8], x: i32, y: i32, width: i32, height: i32, color: u8) -> bool {
    let idx = (y * width + x) as usize;
    (x - 1 >= 0     && img[idx - 1] == color) &&
    (y - 1 >= 0     && img[idx - width as usize] == color) &&
    (x + 1 < width  && img[idx + 1] == color) &&
    (y + 1 < height && img[idx + width as usize] == color)
}
...
fn visit_pixel(
    ...
    facet.point_count += 1;
    if !match_all_around(img, x, y, width, height, facet_color) {
        ...
        loop {
            ...
            if last_row_length != 0 && is_blocked(x, y, width, height, visited, img, facet_color) {
                loop {
                    ...
                    x += 1;
                    if !is_blocked(x, y, width, height, visited, img, facet_color) { break; }
                }
                if last_row_length == 0 { break; }
                scan_x = x;
            } else {
            ...
            if row_length < last_row_length {
                ...
                while tsx < end {
                    if !is_blocked(tsx, y, width, height, visited, img, facet_color) {
                        stack.push((tsx, y));
                    }
                    tsx += 1;
                }
            ...
            y += 1;
            if last_row_length == 0 || y >= height { break; }
        }
    }
}
    ...
    for &n_idx in &neighbours {
        ...
        for i in 0..bp_count {
            ...
            if dist < min_dist {
                ...
                if cd_idx < color_distances.len() {
                    let cd = color_distances[cd_idx];
                    if cd < min_color_dist {
                        min_color_dist = cd;
                        closest = n_idx as i32;
                    }
                }
...
fn rebuild_changed_neighbour_facets(
    ...
    let neighbours: Vec<u32> = facets.get(facet_id as usize)
        .and_then(|f| f.as_ref())
        .and_then(|f| f.neighbour_facets.clone())
        .unwrap_or_default();
    ...
    for &n_idx in &neighbours {
        ...
        if new_facet.point_count == 0 {
            facets[n_idx as usize] = None;
        } else {
            facets[n_idx as usize] = Some(new_facet);
        }
    }

    for &idx in &changed_set {
        if let Some(Some(ref mut f)) = facets.get_mut(idx as usize) {
            f.neighbour_facets = None;
            f.neighbour_facets_is_dirty = true;
        }
    }
}
...
fn delete_facet(
    ...
    let has_neighbours = facets.get(facet_id as usize)
        .and_then(|f| f.as_ref())
        .and_then(|f| f.neighbour_facets.as_ref())
        .map(|n| !n.is_empty())
        .unwrap_or(false);
        ...
        for i in bbox.min_x..=bbox.max_x {
            ...
            if idx < facet_map.len() && facet_map[idx] == facet_id {
                let closest = get_closest_neighbour_for_pixel(
                    facet_id, facets, i, j,
                    color_distances, n_colors, facet_map, width, height,
                );
                if closest >= 0 {
...
fn get_facets(img: &mut Vec<u8>, facet_map: &mut Vec<u32>, width: u32, height: u32) -> Vec<Option<Facet>> {
    ...
    for f in facets.iter_mut() {
        if let Some(ref mut facet) = f {
            build_facet_neighbour(facet, &fm, width, height);
        }
    }
    ...
    while facet_count > maximum_facets as usize {
        ...
        for (i, f) in facets.iter().enumerate() {
            if let Some(ref facet) = f {
                if facet.point_count < min_count {
                    min_count = facet.point_count;
                    min_id = i as u32;
                }
            }
        }
...
pub extern "C" fn reduce_facets(
    img_ptr: *mut u8,
    facet_map_ptr: *mut u32,
    width: u32, height: u32,
    smaller_than: u32,
    maximum_facets: u32,
    remove_large_to_small: u32,
    color_dist_ptr: *const f64,
    n_colors: u32,
) {
    let size = (width * height) as usize;
